export default function SetupList({ title, icon, items }) {
  return (
    <div className="glassmorphism rounded-2xl p-8 border border-slate-800/60 hover:border-amber-500/30 transition-all duration-300">
        ...
        {items.map((item, index) => (
          <div key={index} className="flex justify-between items-center group">
            <div className="flex flex-col">
              <span className="text-slate-300 font-medium tracking-wide group-hover:text-amber-400 transition-colors">
                {item.name}
              </span>
            ...
            {item.link && (
              <a 
                ...
                className="text-slate-600 hover:text-amber-400 transition-colors p-2"
              >
